#!/usr/bin/env bash
# ct — Multi-session Claude Code workspace launcher (vitruvius)
...
#
# Source in ~/.zshrc:  source "$VITRUVIUS_ROOT/scripts/ct.sh"

ct() {
  if [[ -z "$VITRUVIUS_ROOT" ]]; then
    echo "ct: VITRUVIUS_ROOT not set. Run: source ~/.claude/vitruvius.env" >&2
    return 1
  fi
  ...
  local _node_bin="${VITRUVIUS_NODE_BIN:-$(which node)}"
  local _server_mjs="$VITRUVIUS_ROOT/channel/server.mjs"
  local _prompts_dir="$VITRUVIUS_ROOT/prompts"
  _count=$((_count + 1))
...
PYEOF
  ...
  for _entry in \
...

    {
      ...
      printf "export VITRUVIUS_ROOT='%s'\n" "$VITRUVIUS_ROOT"
    ...
    if [[ $_continue -eq 0 ]]; then
      if [[ ! -f "$_path/.claude/project-classifier.md" ]]; then
        (sleep 9 && tmux send-keys -t "${_session}:0.0" \
          "Bootstrap required: .claude/project-classifier.md is missing. Brief Spartacus to analyze this project and generate it from $VITRUVIUS_ROOT/templates/project-classifier.md. Analyze package.json scripts, Makefiles, docker-compose, and existing dev scripts to fill the Dev Environment section. Block all other routing until complete." \
          Enter) &
      else
