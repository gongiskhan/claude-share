...
description: Build structured prompts for the /ralph-loop:ralph-loop command using Claude Code agent teams. Use when user asks to create a ralph-loop prompt, build an agentic loop task, automate a feature with browser verification, or run a task with playwright-cli testing. Always spawns agent teams with specialized roles for implementation, testing, UI/UX, and documentation.
---
...
4. **A tester agent is ALWAYS required.** It uses the `/e2e-testing` skill. For web apps it uses the `playwright-cli` CLI, for Electron apps it uses `mcp__electron__*` tools. For apps requiring login, the tester should first try connecting to an existing browser session via CDP (`playwright-cli --cdp 9222`), then try loading saved auth state (`playwright-cli state load`), and if neither works, open a headed browser (`playwright-cli open <url> --headed`) and ask the user to log in before continuing. It iterates with other agents until ALL tests pass - task features AND related area regressions. Screenshots are required as proof. The tester is the critical gate for completion.
...
6. **For Capacitor apps, do NOT use playwright-cli for testing.** Use simulator screenshots instead.
...
For Electron apps, the tester uses `mcp__electron__*` MCP tools instead of playwright-cli. Include this context:

- This is an Electron app - use `mcp__electron__*` tools for testing, NOT playwright-cli
...
- Do NOT use playwright-cli for testing - use simulator screenshots and user verification
...
HARD RULES:
  ...
  - Capacitor: no playwright-cli, use simulator screenshots
